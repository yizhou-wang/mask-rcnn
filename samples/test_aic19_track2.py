...
DATA_BASE_DIR = "/mnt/disk2/AIC19/aic19-track2-reid"
IMAGE_DIR = os.path.join(DATA_BASE_DIR, "image_test")
OUTPUT_PATH = os.path.join(DATA_BASE_DIR, "image_test_output_masks")
...
with open(DATA_BASE_DIR + "/aic19_track2_test_dets.txt", "w") as f:
  f.seek(0)
  f.truncate()
  for imgname in sorted(imglist):
    ...
    if max_id is not None:
      ...
      np.save(OUTPUT_PATH + "/%06d.npy"%int(frame_id), max_mask)
      ...
      # visualize.display_results(image, r['rois'], r['masks'], r['class_ids'], class_names, r['scores'], save_dir="../masks", img_name="%06d"%int(frame_id)+".png", display_img=False)
      f.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (frame_id, -1, x, y, w, h, str("%.4f" % max_score), class_names[max_id], -1, -1))
